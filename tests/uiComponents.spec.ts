...
test.describe("Form layouts page", () => {
    test.describe.configure({retries: 0})//setting retries only for part of tests
    ...
    test.only("radio buttons", async ({ page }) => {
        ...
        // await usingTheGridForm.getByRole("radio", {name: "Option 1"}).check({force: true}) .check() checks radiobutton / checkbox
...
test.describe("Form layouts page", () => {
    ...
    test('changing theme', async ({ page }) => {
    ...
    test('web tables', async ({ page }) => {
        ...
        await page.locator('input-editor').getByPlaceholder('E-mail').fill("inputed@mail.com")
        await page.locator(".nb-checkmark").click()
        await expect(targetRowById.locator('td').nth(5)).toHaveText("inputed@mail.com")
...
test('slider', async ({page}) => {
    ...
    await tempGauge.evaluate( node => {
        node.setAttribute('cx', '232.620')
        node.setAttribute('cy', '232.620')
    })
